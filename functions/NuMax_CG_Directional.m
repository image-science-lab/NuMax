function [P_k1, r_rank, M, secMinMax, L_k1, q_k1, Lambda_k1, w_k1] = NuMax_CG_directional(data, train_label, delta, opt)
%%%%%%%%%%%%%%
% Computes low-dimensional isometric linear embedding of a data matrix
%
%  Solves the following optimization problem
%  P_k1 = arg max trace(P)   s.t   
%  \forall, train_label(i) \ne train_label(j),  1-delta <= v_ij^T P v_ij  
%  \forall, train_label(i)  =  train_label(j),             v_ij^T P v_ij  <= 1+delta 
%
...
% train_labels - a Tx1 vector of class labels
% delta - isometry slackness. see formulation above
...
pm_val = train_label(idx1) == train_label(idx2); pm_val = 2*pm_val(:)-1;
...
for ll=1:opt.outermost_iterations
    ...
    if (ll > 5)
        ...
        keepidx =  find(pm_val.*(q_k - 1) > delta*.99);
        
        fprintf('Keeping %d \n', length(keepidx));
        secants = secants(:, keepidx);
        pm_val = pm_val(keepidx);
        ...
        while (sIter < opt.num_cg_steps) && (size(secNew, 2) < opt.max_cg_secants)
            ...
            pmtmp = train_label(idx)*ones(1, length(idx));
            pmtmp = pmtmp == pmtmp';
            pmtmp = 2*pmtmp-1;
            pmtmp = pmtmp(upp_lep);
            
%            keep_idx = find( abs(qval-1) > delta);
            keep_idx =  find(pmtmp.*(qval - 1) > delta);
            if (length(keep_idx) > opt.max_cg_secants)
               [junk, k_idx] = sort( pmtmp(keep_idx).*(qval(keep_idx) - 1), 'descend');
               keep_idx = keep_idx(k_idx(1:opt.max_cg_secants));
            end
            
            
            idx1 = idx1(keep_idx); idx2 = idx2(keep_idx);
            pmtmp = pmtmp(keep_idx);
        ...
        secMinMax = [secMinMax; pmNew.*(lala-1)];
        
        fprintf('Adding %d secants. Total time: %4d. Dev %1.2f \n', size(secNew, 2), round(tim1), max(pmNew.*(lala-1)));
    ...
    for qq=1:opt.outer_iterations
        ...
        for kk=1:opt.inner_iterations
            ...
            %inf-norm solver
            tic
            q_k1 = solve_infinity_norm_problem(b, funA_L_k+w_k, delta, pm_val);
            t1 = t1 + toc;
...
function x = solve_infinity_norm_problem(b, y, delta, pm_val)
